 ...
 */
async function callAIService({ prompt, systemInstruction, type }) {
    const apiKey = env_1.env.CLAUDE_API_KEY || env_1.env.OPENAI_API_KEY || env_1.env.GEMINI_API_KEY || env_1.env.AI_API_KEY;
    if (apiKey && apiKey !== 'dev_ai_key') {
        try {
            ...
            if (env_1.env.GEMINI_API_KEY) {
                ...
                const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${env_1.env.GEMINI_API_KEY}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
