...
import { User, Device, Course, Lesson, Bundle, Coupon, Order, UserCourseAccess, LiveSession } from '../models';
...
async function seed() {
  ...
  try {
    ...
    const samplePdfPath = path.join(uploadsDir, 'sample_guide.pdf');
    ...
    const sampleVideoPath = path.join(uploadsDir, 'sample_lecture.mp4');
    // Write 100 bytes of dummy mp4 stream data
    fs.writeFileSync(sampleVideoPath, Buffer.alloc(100, 'MOCK_VIDEO_BINARY_STREAM'));
    ...
    // Course 1 Bundles
    const c1_video = await Bundle.create({
      courseId: course1._id,
    ...
    const c1_pdf = await Bundle.create({
      courseId: course1._id,
    ...
    const c1_combo = await Bundle.create({
      courseId: course1._id,
    ...
    const c2_pdf = await Bundle.create({
      courseId: course2._id,
    ...
    const c2_combo = await Bundle.create({
      courseId: course2._id,
    ...
    // Course 3 Bundles
    const c3_video = await Bundle.create({
      courseId: course3._id,
    ...
    const c3_combo = await Bundle.create({
      courseId: course3._id,
