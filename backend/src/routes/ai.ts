...
import { z } from 'zod';
import { AiJob, Note, Bookmark, Progress, Lesson, Course, User } from '../models';
import { authenticate } from '../middleware/auth';
import { callAIService } from '../services/ai.service';
import { PDFDocument, StandardFonts, rgb } from 'pdf-lib';

export default async function aiNotesRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/ai/summarise/:lessonId', { preHandler: [authenticate] }, async (request, reply) => {
    const { lessonId } = request.params as any;
    const { userId } = request.user!;

    const lesson = await Lesson.findById(lessonId);
    if (!lesson) return reply.status(404).send({ statusCode: 404, error: 'Not Found', message: 'Lesson not found' });
  ...
  // RAG Q&A
  fastify.post('/ai/ask', { preHandler: [authenticate] }, async (request, reply) => {
  ...
  // Polish Notes
  fastify.post('/ai/polish-notes', { preHandler: [authenticate] }, async (request, reply) => {
    const schema = z.object({
    ...
    const polishedText = await callAIService({
      type: 'NOTES_POLISH',
      prompt,
  ...
  // Mindmap Generation
  fastify.post('/ai/mindmap', { preHandler: [authenticate] }, async (request, reply) => {
    const schema = z.object({
    ...
    const mindmapStr = await callAIService({
      type: 'MINDMAP',
      prompt,
  ...
  // Poll Job Status
  fastify.get('/ai/jobs/:id', { preHandler: [authenticate] }, async (request, reply) => {
    const { id } = request.params as any;
    const job = await AiJob.findOne({ _id: id, userId: request.user!.userId });
    if (!job) {
  ...
  fastify.get('/notes', { preHandler: [authenticate] }, async (request, reply) => {
    const notes = await Note.find({ userId: request.user!.userId })
  ...
  // Get note detail
  fastify.get('/notes/:id', { preHandler: [authenticate] }, async (request, reply) => {
    const { id } = request.params as any;
    const note = await Note.findOne({ _id: id, userId: request.user!.userId })
      .populate('courseId', 'title')
      .populate('lessonId', 'title type');
  ...
  // Create Note
  fastify.post('/notes', { preHandler: [authenticate] }, async (request, reply) => {
    const schema = z.object({
      courseId: z.string(),
      lessonId: z.string(),
      title: z.string().default('Untitled Note'),
      content: z.any().optional(),
      mindmapJson: z.any().optional(),
    });
  ...
  // Update Note
  fastify.put('/notes/:id', { preHandler: [authenticate] }, async (request, reply) => {
    const { id } = request.params as any;
    const schema = z.object({
  ...
  // Delete Note
  fastify.delete('/notes/:id', { preHandler: [authenticate] }, async (request, reply) => {
    const { id } = request.params as any;
    const note = await Note.findOneAndDelete({ _id: id, userId: request.user!.userId });
    if (!note) {
      return reply.status(404).send({ statusCode: 404, error: 'Not Found', message: 'Note not found' });
    }
  ...
  fastify.get('/notes/:id/export', { preHandler: [authenticate] }, async (request, reply) => {
    const { id } = request.params as any;
    ...
    const courseTitle = (note.courseId as any)?.title || 'Course';
    const lessonTitle = (note.lessonId as any)?.title || 'Lesson';
    ...
    if (format === 'pdf') {
      ...
      page.drawText(noteTitle.toUpperCase(), { x: 50, y: 740, size: 20, font: boldFont, color: rgb(0/255, 110/255, 255/255) });
      ...
      for (const line of lines) {
        if (currentY < 50) break; // prevent overflowing page
  ...
  // Create Bookmark
  fastify.post('/bookmarks', { preHandler: [authenticate] }, async (request, reply) => {
    const schema = z.object({
      lessonId: z.string(),
      timestampSecs: z.number().min(0),
  ...
  fastify.get('/bookmarks', { preHandler: [authenticate] }, async (request, reply) => {
    const bookmarks = await Bookmark.find({ userId: request.user!.userId })
  ...
  // Delete Bookmark
  fastify.delete('/bookmarks/:id', { preHandler: [authenticate] }, async (request, reply) => {
    const { id } = request.params as any;
    const bookmark = await Bookmark.findOneAndDelete({ _id: id, userId: request.user!.userId });
    if (!bookmark) {
  ...
  // Update progress
  fastify.put('/progress/:lessonId', { preHandler: [authenticate] }, async (request, reply) => {
    const { lessonId } = request.params as any;
    const schema = z.object({
      watchedSecs: z.number().min(0),
      totalSecs: z.number().min(1),
    ...
    const progress = await Progress.findOneAndUpdate(
      { userId: request.user!.userId, lessonId },
      {
  ...
  fastify.get('/progress/:courseId', { preHandler: [authenticate] }, async (request, reply) => {
    const { courseId } = request.params as any;
    const { userId } = request.user!;

    // Find all lessons in this course
    const lessons = await Lesson.find({ courseId }, '_id title');
